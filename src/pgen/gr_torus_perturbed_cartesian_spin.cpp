...
AthenaArray<Real> t_orbits,x1_orbits,x2_orbits,y1_orbits,y2_orbits,z1_orbits,z2_orbits;
AthenaArray<Real> v1x_orbits,v2x_orbits,v1y_orbits,v2y_orbits,v1z_orbits,v2z_orbits;
AthenaArray<Real> a1x_orbits,a2x_orbits,a1y_orbits,a2y_orbits,a1z_orbits,a2z_orbits;
...
void set_orbit_arrays(std::string orbit_file_name){
    ...
    x1_orbits.NewAthenaArray(nt);
    y1_orbits.NewAthenaArray(nt);
    z1_orbits.NewAthenaArray(nt);

    x2_orbits.NewAthenaArray(nt);
    y2_orbits.NewAthenaArray(nt);
    z2_orbits.NewAthenaArray(nt);

    a1x_orbits.NewAthenaArray(nt);
    a1y_orbits.NewAthenaArray(nt);
    a1z_orbits.NewAthenaArray(nt);

    a2x_orbits.NewAthenaArray(nt);
    a2y_orbits.NewAthenaArray(nt);
    a2z_orbits.NewAthenaArray(nt);


    v1x_orbits.NewAthenaArray(nt);
    v1y_orbits.NewAthenaArray(nt);
    v1z_orbits.NewAthenaArray(nt);


    v2x_orbits.NewAthenaArray(nt);
    v2y_orbits.NewAthenaArray(nt);
    v2z_orbits.NewAthenaArray(nt);
    ...
    fread( &x1_orbits(it), sizeof( Real ), 1, input_file );
    fread( &y1_orbits(it), sizeof( Real ), 1, input_file );
    fread( &z1_orbits(it), sizeof( Real ), 1, input_file );

    fread( &x2_orbits(it), sizeof( Real ), 1, input_file );
    fread( &y2_orbits(it), sizeof( Real ), 1, input_file );
    fread( &z2_orbits(it), sizeof( Real ), 1, input_file );

    fread( &a1x_orbits(it), sizeof( Real ), 1, input_file );
    fread( &a1y_orbits(it), sizeof( Real ), 1, input_file );
    fread( &a1z_orbits(it), sizeof( Real ), 1, input_file );

    fread( &a2x_orbits(it), sizeof( Real ), 1, input_file );
    fread( &a2y_orbits(it), sizeof( Real ), 1, input_file );
    fread( &a2z_orbits(it), sizeof( Real ), 1, input_file );


    fread( &v1x_orbits(it), sizeof( Real ), 1, input_file );
    fread( &v1y_orbits(it), sizeof( Real ), 1, input_file );
    fread( &v1z_orbits(it), sizeof( Real ), 1, input_file );

    fread( &v2x_orbits(it), sizeof( Real ), 1, input_file );
    fread( &v2y_orbits(it), sizeof( Real ), 1, input_file );
    fread( &v2z_orbits(it), sizeof( Real ), 1, input_file );
...
void get_prime_coords(Real x, Real y, Real z, Real t, Real *xprime, Real *yprime, Real *zprime, Real *rprime, Real *Rprime){
  ...
  Real ax,ay,az,a_mag;
  get_secondary_spin(t,&ax,&ay,&az,&a_mag);
...
//   Real v2x = 0.0;
//   Real v2y =  2.0*PI*Omega_bh2 * r_bh2 * std::cos(2.0*PI*Omega_bh2 * t);
//   Real v2z = -2.0*PI*Omega_bh2 * r_bh2 * std::sin(2.0*PI*Omega_bh2 * t);
//   if (std::fabs(zprime)<SMALL) zprime= SMALL;
    ...
    AthenaArray<Real> &g)
{
  ...
  Real a1x, a1y, a1z, a1;
  get_primary_spin(t,&a1x,&a1y, &a1z, &a1);

  Real a2x, a2y, a2z, a2;
  get_primary_spin(t,&a2x,&a2y, &a2z, &a2);
...
/// prevent metric from gettin nan sqrt(-gdet)
  ...
  // Real v2z = 0.0;
  // Real v2x =  Omega_bh2 * r_bh2 * std::cos(Omega_bh2 * (t-t0));
  // Real v2y = -Omega_bh2 * r_bh2 * std::sin(Omega_bh2 * (t-t0));


  // Real v2z = 0.0;
  // Real v2x = -Omega_bh2 * r_bh2 * std::sin(Omega_bh2 * (t-t0));
  // Real v2y = Omega_bh2 * r_bh2 * std::cos(Omega_bh2 * (t-t0));
...
/// prevent metric from gettin nan sqrt(-gdet)
  ...
  Real dLorentz_dt = std::pow(Lorentz,3.0) * (v2x*ax_bh2 + v2y*ay_bh2 + v2z*az_bh2);
  ...
  dLambda_dt(I01) = -v2x*dLorentz_dt - Lorentz*ax_bh2;
  dLambda_dt(I02) = -v2y*dLorentz_dt - Lorentz*ay_bh2;
  dLambda_dt(I03) = -v2z*dLorentz_dt - Lorentz*az_bh2;
  ...
  // l_lowerprime[0] = Lorentz * (l0 - v2x * l1 - v2y * l2 - v2z * l3);
  // l_lowerprime[3] = Lorentz * (l3 - v2 * l0);

  //These assuem gamma = 1.  Much more complicated if not

  // l_lowerprime[0] = (l0 - v2x * l1 - v2y * l2 - v2z * l3);
  // l_lowerprime[1] = (l1 - v2x * l0);
  // l_lowerprime[2] = (l2 - v2y * l0);
  // l_lowerprime[3] = (l3 - v2z * l0);
  ...
  // g(I00) += -2.0 * ( v2x * fprime * l_lowerprime[0]*l_lowerprime[1] 
  //                  + v2y * fprime * l_lowerprime[0]*l_lowerprime[2] 
  //                  + v2z * fprime * l_lowerprime[0]*l_lowerprime[3])
  //           + SQR(v2x) * fprime * l_lowerprime[1]*l_lowerprime[1]
  //           + SQR(v2y) * fprime * l_lowerprime[2]*l_lowerprime[2] 
  //           + SQR(v2z) * fprime * l_lowerprime[3]*l_lowerprime[3];
  // g(I01) += - v2x * fprime * l_lowerprime[1]*l_lowerprime[1];
  // g(I02) += - v2y * fprime * l_lowerprime[2]*l_lowerprime[2];
  // g(I03) += - v2z * fprime * l_lowerprime[3]*l_lowerprime[3];
...
//   // dl0prime_dx1 = (dl0_dx1_tmp - v2x * dl1_dx1_tmp - v2y * dl2_dx1_tmp - v2z * dl3_dx1_tmp); 
//   // dl0prime_dx2 = (dl0_dx2_tmp - v2x * dl1_dx2_tmp - v2y * dl2_dx2_tmp - v2z * dl3_dx2_tmp); 
//   // dl0prime_dx3 = (dl0_dx3_tmp - v2x * dl1_dx3_tmp - v2y * dl2_dx3_tmp - v2z * dl3_dx3_tmp);  


//   // dl1prime_dx1 = (dl1_dx1_tmp - v2x * dl0_dx1_tmp); 
//   // dl1prime_dx2 = (dl1_dx2_tmp - v2x * dl0_dx2_tmp); 
//   // dl1prime_dx3 = (dl1_dx3_tmp - v2x * dl0_dx3_tmp); 


//   // dl2prime_dx1 = (dl2_dx1_tmp - v2y * dl0_dx1_tmp); 
//   // dl2prime_dx2 = (dl2_dx2_tmp - v2y * dl0_dx2_tmp); 
//   // dl2prime_dx3 = (dl2_dx3_tmp - v2y * dl0_dx3_tmp); 

//   // dl3prime_dx1 = (dl3_dx1_tmp - v2z * dl0_dx1_tmp); 
//   // dl3prime_dx2 = (dl3_dx2_tmp - v2z * dl0_dx2_tmp); 
//   // dl3prime_dx3 = (dl3_dx3_tmp - v2z * dl0_dx3_tmp); 
...
//   Real dX_dt = dLambda_dt(I11) * (x - x_bh2) - Lambda(I11) * v2x + 
//                dLambda_dt(I12) * (y - y_bh2) - Lambda(I12) * v2y + 
//                dLambda_dt(I13) * (z - z_bh2) - Lambda(I13) * v2z ;

//   Real dY_dt = dLambda_dt(I12) * (x - x_bh2) - Lambda(I12) * v2x + 
//                dLambda_dt(I22) * (y - y_bh2) - Lambda(I22) * v2y + 
//                dLambda_dt(I23) * (z - z_bh2) - Lambda(I23) * v2z ;

//   Real dZ_dt = dLambda_dt(I13) * (x - x_bh2) - Lambda(I13) * v2x + 
//                dLambda_dt(I23) * (y - y_bh2) - Lambda(I23) * v2y + 
//                dLambda_dt(I33) * (z - z_bh2) - Lambda(I33) * v2z ;
