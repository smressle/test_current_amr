...
int RefinementConditiona0_new(MeshBlock *pmb);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if(adaptive==true) EnrollUserRefinementCondition(RefinementConditiona0_new);
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      ...
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        if (Be>0 || (radius<rh) || (rprime<rhprime) || T_old<Target_Temperature){
          ...
          // T_new = Target_Temperature * ( SQR(std::tan(atan_arg)) + 1.0);
...
/// prevent metric from gettin nan sqrt(-gdet)
  ...
  if (take_derivatives){
      ...
      if (std::isnan(f) || std::isnan(r) || std::isnan(sqrt_term) || std::isnan (df_dx1) || std::isnan(df_dx2)){
        fprintf(stderr,"ISNAN in metric\n x y y: %g %g %g r: %g \n xyz_old: %g %g %g \n th: %g phi: %g \n",x,y,z,r,x_old,y_old,z_old,th,phi);
        exit(0);
      }
