...
void Coordinates::Metric(Real t, Real x1, Real x2, Real x3, ParameterInput *pin,
...
//----------------------------------------------------------------------------------------
// Function for implementing user-defined metric without access to pin
// Inputs:
//   x1,x2,x3: spatial location of point
//   pin: pointer to runtime inputs
// Outputs:
...
void Coordinates::MetricWithoutPin(Real t, Real x1, Real x2, Real x3,
    AthenaArray<Real> &g, AthenaArray<Real> &g_inv, AthenaArray<Real> &dg_dx1,
    AthenaArray<Real> &dg_dx2, AthenaArray<Real> &dg_dx3, AthenaArray<Real> &dg_dt,bool take_derivatives) {

  // if (t != pmy_block->pmy_mesh->metric_time) fprintf(stderr,"ERROR...Metric time not equal to called time in coordinates.cpp : %g %g \n",t,pmy_block->pmy_mesh->metric_time);

  if (METRIC_EVOLUTION) pmy_block->pmy_mesh->UserMetricWithoutPin_(t,x1, x2, x3, g, g_inv, dg_dx1, dg_dx2, dg_dx3,dg_dt,take_derivatives);
  else pmy_block->pmy_mesh->UserMetricWithoutPin_(0.0,x1, x2, x3, g, g_inv, dg_dx1, dg_dx2, dg_dx3,dg_dt,take_derivatives);
  return;
}
