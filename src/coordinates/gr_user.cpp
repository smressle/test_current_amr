...
#pragma omp simd
      for (int i=is; i<=ie; ++i) {
        ...
        if ((pmy_block->gid ==3515 || pmy_block->gid ==3518) &&  (i==17) && (j==17) && (k==2)) {
          fprintf(stderr,"Energy Source Term gid: %d : %g E: %g \n t: %g metric_t: %g s_1 s_2 s_3: %g %g %g \n m_1 m_2 m_3: %g %g %g \n",pmy_block->gid , s_E,E,pmy_block->pmy_mesh->time,pmy_block->pmy_mesh->metric_time,
      ...
      for (int i = ill; i <= iuu; ++i) {
        ...
        if (not coarse_flag ) {
          ...
          for (int n_cons=IDN; n_cons <= IEN; ++n_cons){
            ...
            //facp1 = sqrt_minus_det_old/std::sqrt(-det);
            if (is_half_time_step) pmb->phydro->u1(n_cons,k,j,i) *=fac;
          }
    ...
    for (int k = kll; k <= kuu; ++k) {
      for (int j = jll; j <= juu; ++j) {
        for (int i = ill; i <= iuu+1; ++i) {
          ...
          if (MAGNETIC_FIELDS_ENABLED and coarse_flag) pmb->pfield->coarse_b_.x1f(k,j,i) *= fac;
    ...
    for (int k = kll; k <= kuu; ++k) {
      for (int j = jll; j <= juu+1; ++j) {
        for (int i = ill; i <= iuu; ++i) {
          ...
          if (MAGNETIC_FIELDS_ENABLED and coarse_flag) pmb->pfield->coarse_b_.x2f(k,j,i) *= fac;
    ...
    for (int k = kll; k <= kuu+1; ++k) {
      for (int j = jll; j <= juu; ++j) {
        for (int i = ill; i <= iuu; ++i) {
          ...
          if (MAGNETIC_FIELDS_ENABLED and coarse_flag) pmb->pfield->coarse_b_.x3f(k,j,i) *= fac;
