  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      for(int i=is; i<=ie; i++) {
        ...
        if (std::fabs(pmb->pcoord->x1v(i)+188.0)<1e-2 && std::fabs(pmb->pcoord->x2v(j)+188.0)<1e-2 &&  std::fabs(pmb->pcoord->x3f(k)-24.0)<1e-2){
          fprintf(stderr, "B above interface ijk: %d %d %d B: %g \n ", i,j,k,b.x3f(k,j,i));
        }   
        if (std::fabs(pmb->pcoord->x1v(i)+188.0)<1e-2 && std::fabs(pmb->pcoord->x2v(j)+188.0)<1e-2 &&  std::fabs(pmb->pcoord->x3f(k+1)-24.0)<1e-2){
